...
simulate_catch <- function(removals, dem_params, naa, options){
    ...
    if(options$removals_input == "catch"){
        F_f <- findF_multifleet(
            removals,
            naa = naa,
            waa = dem_params$waa,
            mort = dem_params$mort,
            selex = dem_params$sel,
            f_guess = 0.02
        )
    }else{
        F_f <- removals
    ...
    #     ret_faa <- retained_F(F_f[f], dp.f$sel, dp.f$ret)
    ...
    #     disc_caa_tmp[,,,,f] <- catch_at_age(disc_daa, naa, dp.f$waa, dp.f$mort)
    #     caa_tmp[,,,,f] <- land_caa_tmp[,,,,f] + disc_caa_tmp[,,,,f]
    #     F_f_tmp[,,,,f] <- F_f
    # }

    ret_faa <- retained_F(F_f, dem_params$sel, dem_params$ret)
    disc_faa <- discard_F(dem_params$dmr, dem_params$sel, dem_params$ret)
    faa_tmp <- ret_faa + disc_faa

    land_caa_tmp <- catch_at_age(ret_faa, naa, dem_params$waa, dem_params$mort)
    disc_caa_tmp <- catch_at_age(disc_faa, naa, dem_params$waa, dem_params$mort)
    caa_tmp <- land_caa_tmp + disc_caa_tmp
    F_f_tmp <- F_f
