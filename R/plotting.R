...
plot_mort <- function(mort, is_dmr=FALSE){
    ...
    plot <- ggplot2::ggplot(mort_df, ggplot2::aes(x=age, y=value, color=sex, linetype=time_block)) +
      ...
      ggplot2::labs(x="Age",
                    y="Instanteous Mortality",
                    color="Sex",
                    linetype="Time Block",
                    title=ifelse(is_dmr, "Discard Mortality-at-Age", "Natural Mortality-at-Age")) +
      ggplot2::theme_bw()
...
plot_selret <- function(selret, is_selectivity=TRUE){
    ...
    plot <- ggplot2::ggplot(selex_df, ggplot2::aes(x=age, y=value, color=sex, linetype=time_block)) +
      ...
      ggplot2::scale_y_continuous(limits=c(0, 1), expand=c(0.01, 0.01)) +
      ggplot2::scale_x_continuous(expand=c(0, 0)) +
      ggplot2::labs(x="Age",
                    y=ifelse(is_selectivity, "Selectivity", "Retention"),
                    color="Sex",
                    linetype="Time Block",
                    title=ifelse(is_selectivity, "Selectivity", "Retention"))+
      ggplot2::theme_bw()
    ...
    if(dimensions$nregions > 1 & dimensions$nfleets <= 1){
        ...
        plot <- plot +
          ggplot2::facet_grid(rows=vars(region), cols=vars(fleet))
    }
...
plot_ssb <- function(ssb){
    ...
    ssb_df <- tibble::as_tibble(ssb) %>%
        tibble::rownames_to_column("time") %>%
        dplyr::mutate(time=as.numeric(time)) %>%
        tidyr::pivot_longer(c(2), names_to="region", values_to="ssb")
 ...
 plot_bio <- function(bio){
    ...
    bio_df <- tibble::as_tibble(bio) %>%
        tibble::rownames_to_column("time") %>%
        dplyr::mutate(time=as.numeric(time)) %>%
        tidyr::pivot_longer(c(2), names_to="region", values_to="bio")
 ...
 plot_catch <- function(catch){
    ...
    catch_df <- tibble::as_tibble(catch) %>%
        tibble::rownames_to_column("time") %>%
        dplyr::mutate(time=as.numeric(time)) %>%
        tidyr::pivot_longer(c(2), names_to="region", values_to="catch")
 ...
 plot_f <- function(f){
    ...
    f_df <- tibble::as_tibble(f) %>%
        tibble::rownames_to_column("time") %>%
        dplyr::mutate(time=as.numeric(time)) %>%
        tidyr::pivot_longer(c(2), names_to="region", values_to="f")
...
plot_atage <- function(atage){
    ...
    plot <- ggplot2::ggplot(atage_df, ggplot2::aes(x=time, y=age, size=prop, color=prop))+
      ggplot2::geom_point()+
      ggplot2::geom_line(aes(y=avg_age), color='red', show.legend = FALSE)+
      ggplot2::coord_cartesian(expand=TRUE)+
      ggplot2::theme_bw()
