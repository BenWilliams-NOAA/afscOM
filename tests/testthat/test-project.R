test_that("1yr projection w/o movement", {
  ...
  rec[1,1,,1] <- 2*assessment$natage.female["1960",1] *1e6

  tac <- 3114310
  suppressWarnings({
    out_vars <- project(TAC=tac, dem_params=dem_params, prev_naa=naa, recruitment=rec, fleet.props = c(0.7, 0.3), options=model_options)
  })

  total_catch <- apply(out_vars$caa_tmp, 1, sum)
  expect_equal(total_catch, tac, tolerance=1e-6)

  fleet_catch <- apply(out_vars$caa, c(1, 5), sum)
